...
if(isset($_POST['title'])) {
    ...
    if($isError) {
        ...
        if($conn && !$isError) {
            try {
                $sql = 'UPDATE products SET title = :title, descrip = :descrip, price = :price, stock = :stock WHERE id = :id';
                $stmt = $conn->prepare($sql);
                $stmt->execute(['title' => $title, 'descrip' => $description, 'price' => $price, 'stock' => $stock, 'id' => $id]);
